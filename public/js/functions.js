...
function getPageNav(_page, _num, _total) {
  ...
  var nav = [];
  if( _page != 1 ) nav.push({ name: "首页", page: 1, link_page: update_search({ page: 1 }) });

  if( _page > 2 ) nav.push({ name: "上一页", page: _page - 1, link_page: update_search({ page: _page - 1 }) });

  if( _page > 3 ) nav.push({ name: "...", page: null, link_page: null });

  for(var i = _page - 2; i <= _page + 2; i++) {
    if(i > 0 && i <= total_page_num) {
      if(i == _page)
        nav.push({ name: i.toString(), page: null, link_page: null});
      else
        nav.push({ name: i.toString(), page: i, link_page: update_search({ page: i }) });
    }
  }

  if( total_page_num - _page > 3) nav.push({ name: "...", page: null, link_page: null});

  if( total_page_num - _page > 1) nav.push({ name: "下一页", page: _page + 1, link_page: update_search({ page: _page + 1 }) });

  if( total_page_num - _page > 0) nav.push({ name: "尾页", page: total_page_num, link_page: update_search({ page: total_page_num }) });
